...
// Create the "updated" directory if it doesn't exist
if (!fs.existsSync(updatedDir)) {
  fs.mkdirSync(updatedDir);
}

// Function to process each video file
const processVideo = (videoFile, callback) => {
...
fs.readdir(videoDir, (err, files) => {
  ...
  const queue = async.queue((videoFile, callback) => {
    processVideo(videoFile, callback);
  }, 4);

  // Push video files to the queue
  videoFiles.forEach(videoFile => {
    queue.push(videoFile, (err) => {
      if (err) {
  ...
  // All items have been pushed to the queue
  queue.drain(() => {
